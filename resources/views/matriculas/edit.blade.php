@extends('templates.main', ['titulo' => 'Alterar Matrícula'])
...
@section('conteudo')
    <form action="{{ route('matricula.update', ['id' => $dados->id]) }}" method="POST">
        @csrf
        @method('PUT')
        <div class="row">
            <div class="col">
                <div class="form-floating mb-3">
                    <label for="disciplina_id">Disciplina: {{ $dados->disciplina->nome }}</label>
                </div>
            </div>
        </div>
        <div class="row">
                            ...
                            <tr>
                                ...
                                <td>
                                    <input type="checkbox" name="disciplinas[]" value="{{ $disciplina->id }}"
                                        {{ in_array($disciplina->id, $matriculasAluno) ? 'checked' : '' }}>
                                </td>
                            </tr>
                        @endforeach
                    </tbody>
